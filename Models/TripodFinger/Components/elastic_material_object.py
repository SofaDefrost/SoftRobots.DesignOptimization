...
class ElasticMaterialObject(Sofa.Prefab):
    ...
    def init(self):
        ...
        if self.solverName.value == '':
            ...
            plugins.append('Sofa.Component.LinearSolver.Direct')
            self.solver = self.addObject('SparseLDLSolver', name="solver", template="CompressedRowSparseMatrixMat3x3d")
        ...
        plugins.append('Sofa.Component.SolidMechanics.FEM.Elastic')
        if self.topoMesh.value == "tetrahedron":
